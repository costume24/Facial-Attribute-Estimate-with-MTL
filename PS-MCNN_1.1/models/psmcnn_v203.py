...
def conv_3x3_bn_prelu(inp, oup, stride=1):
    return nn.Sequential(nn.Conv2d(inp, oup, 3, stride, 1, bias=False), nn.BatchNorm2d(oup), nn.PReLU())
...
class psnet(nn.Module):
    def __init__(self, use_1x1=True, prelu='no', reduction=4, scale1=1.0, scale2=1.0, asy='no',learn='no',use_se=True):
        super().__init__()
        if prelu == 'yes':
        ...
        self.s_fc = nn.ModuleList([nn.Linear(3840, 512), nn.Linear(512, 512)])  # (2,)，s支路的2个FC层
        self.output = []  # (4,), 4个支路的输出
        self.tconv_1x1 = nn.ModuleList()  # (4,4)，用1x1卷积进行降维，取代原来的取前32个通道
        for _ in range(4):
            tmp = nn.ModuleList([conv1(32, 32), conv1(64, 32), conv1(128, 32), conv1(256, 32)])
            self.tconv_1x1.append(tmp)
        self.sconv_3x3 = nn.ModuleList()
        for _ in range(4):
            tmp = nn.ModuleList([conv3(32, 32), conv3(64, 32), conv3(128, 32), conv3(256, 32)])
        ...
        self.iablock_s = nn.ModuleList([
            Block(32, 32, reduction, scale1, scale2, use_se, learn=learn),
            Block(160, 64, reduction, scale1, scale2, use_se, learn=learn),
            Block(192, 128, reduction, scale1, scale2, use_se=use_se, asy=self.asy, learn=learn),
            Block(256, 256, reduction, scale1, scale2, use_se=use_se, asy=self.asy, learn=learn),
            Block(384, 128, reduction, scale1, scale2, use_se, learn=learn)
        ])
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(nn.Linear(channel, int(channel / reduction), bias=False), nn.ReLU(inplace=True),
                                nn.Linear(int(channel / reduction), channel, bias=False), nn.Sigmoid())
