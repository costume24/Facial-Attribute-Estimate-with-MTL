...
class psnet(nn.Module):
    def __init__(self, use_1x1=True, prelu='no',ratio=0.25, num_attributes=40, input_size=224):
        ...
        self.cbam_list = nn.ModuleList()  # (4,5), 仅在t支路上加入se模块
        for _ in range(4):
            tmp = nn.ModuleList(
                [CBAM(64),
                 CBAM(128),
        ...
        self.cbam_list_s = nn.ModuleList(
            [CBAM(160),
             CBAM(192),
             CBAM(256),
             CBAM(384)])  # (4,), 仅在s支路上加入se模块
