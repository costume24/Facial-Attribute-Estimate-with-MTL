...
# out: ['mobilenetv1', 'mobilenetv2', 'resnet50']
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for k in range(40):
            # print(output[j, :, k].size())
            # print(target[j, k].size())
            # print(target[j, k])
            loss_attr[k] += criterion(output[:, :, k], target[:, k].long())
            # loss_single.append(criterion(output[:, :, k], target[:, k]).long())
        ...
        # loss_sum = loss_sum.requires_grad_()
        # loss_sum.backward()
        loss.backward()
        optimizer.step()
