...
image_path = "/media/xuke/SoftWare/BaiduNetdiskDownload/CelebA/Img/img_align_celeba/img_align_celeba"
        ...
        for line in Attr_info:
            ...
            if os.path.isfile(filepath_old):
                if int(info[Attr_type]) == 1:
                    ...
                    shutil.copyfile(filepath_old, filepath_new)
                    count_B += 1
                print("%d: success for copy %s -> %s" %
                      (index, info[Attr_type], filepath_new))
            else:
                print("%d: not found %s\n" % (index, filepath_old))
