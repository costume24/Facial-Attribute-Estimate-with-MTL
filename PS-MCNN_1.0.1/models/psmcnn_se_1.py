...
def se(inp, r=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                         nn.Linear(inp, int(inp / r)), nn.ReLU6(inplace=True),
                         nn.Linear(int(inp / r), inp), nn.Sigmoid())
...
class psnet(nn.Module):
    ...
    def forward(self, input):
        ...
        # for i in range(len(block_5)):
        #     print('1',block_5[i].size())
        # for i in range(len(self.output)):
        #     print('2',self.output[i].size())
        for i in range(4):
